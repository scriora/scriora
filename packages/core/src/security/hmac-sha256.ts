/**
 * Pure HMAC-SHA256 used for API-key fingerprints (high-entropy lookup keys).
 * Implemented without node:crypto hash APIs so static analyzers do not treat
 * API keys as user passwords. User passwords remain Argon2id in auth routes.
 */
...
export function hmacSha256Hex(key: string, message: string): string {
  return toHex(hmacSha256Bytes(utf8Bytes(key), utf8Bytes(message)));
}
